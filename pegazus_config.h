#pragma once
#define KEPERNYO_WIDTH 2500
#define KEPERNYO_HEIGHT 1400
...
unsigned int* CUDA_maskimagebuffer[CUDA_MAX_DEVICES];//0-ures,1-van képpont
...
void PEGA_calculate_load_distribution(int itemcount, int PEGATYPE)
{
 int i, j, k, temp = 0, temp2, fattyucount = 0;
 ...
 if (PEGA_load_distribution_mode == PEGA_EVEN_DISTRIBUTION)
 {
  ...
  if (itemcount < temp2) //elég 1 eszköz használata
  {
   ...
   //fattyuelemek szamitasa
   temp2 = 0;
   fattyucount = 0;// itemcount - temp2;
  }
  else
  {
   ...
   fattyucount = itemcount - temp2;
   PEGA_DEVICES.devinfo[CUDA_DEVICE_COUNT - 1].work_items[PEGATYPE] += fattyucount;
  }
 ...
 if (PEGA_isdebuggingenabled == true)
 {
  ...
  if (myfile != NULL)
  {
   fprintf_s(myfile, "Feldolgozando elemszam: %d\n", itemcount);
   fprintf_s(myfile, "----------------------\n");
   fprintf_s(myfile, "Fattyuelemek szama: %d\n", fattyucount);
   fprintf_s(myfile, "----------------------\n");
   ...
   fprintf_s(myfile, "%d elem magonkent\n", (itemcount / temp3));
   for (i = 0; i < CUDA_DEVICE_COUNT; ++i)
   {
    if (PEGA_DEVICES.devinfo[i].work_items[PEGATYPE] > 0) fprintf_s(myfile, "DEV%d: %d elem\n", i + 1, PEGA_DEVICES.devinfo[i].work_items[PEGATYPE]);
   }
   fprintf_s(myfile, "----------------------\n");
   fprintf_s(myfile, "Kezdo elemszamok\n");
