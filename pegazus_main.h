...
unsigned int D2D_imagebuffer[KEPERNYO_WIDTH * KEPERNYO_HEIGHT];
...
void PEGA_init(HWND hwnd)
{
 ...
 pD2DFactory->CreateHwndRenderTarget(
  ...
  D2D1::HwndRenderTargetProperties(
   hwnd, D2D1::SizeU(KEPERNYO_WIDTH, KEPERNYO_HEIGHT)), &pRT);
 ...
 for (i = 0; i < CUDA_DEVICE_COUNT; ++i)
 {
  ...
  cudaMalloc((void**)&CUDA_imagebuffer[i], KEPERNYO_WIDTH * KEPERNYO_HEIGHT * sizeof(unsigned int));
  cudaMalloc((void**)&CUDA_maskimagebuffer[i], KEPERNYO_WIDTH * KEPERNYO_HEIGHT * sizeof(unsigned int));
 } 
 cudaSetDevice(0);
 cudaMalloc((void**)&CUDA_DEV0_COPY_imagebuffer, KEPERNYO_WIDTH * KEPERNYO_HEIGHT * sizeof(unsigned int));
}
...
void PEGA_merge_down_2D_buffer(void)
{
 ...
 {
  for (i = 1; i < CUDA_DEVICE_COUNT; ++i) //itt az 1. sorszámú eszközzel kezdünk!!!
  {
   cudaMemcpyPeer(CUDA_maskimagebuffer[0], 0, CUDA_maskimagebuffer[i], i, KEPERNYO_WIDTH * KEPERNYO_HEIGHT * sizeof(unsigned int));
   cudaMemcpyPeer(CUDA_DEV0_COPY_imagebuffer, 0, CUDA_imagebuffer[i], i, KEPERNYO_WIDTH * KEPERNYO_HEIGHT * sizeof(unsigned int));
   PEGA_merge_CUDA_images << <CUDA_BLOCKS, CUDA_CORES >> > (KEPERNYO_WIDTH * KEPERNYO_HEIGHT, CUDA_imagebuffer[0], CUDA_maskimagebuffer[0], CUDA_DEV0_COPY_imagebuffer);
...
{
 D2D1_RECT_U terulet;
 cudaMemcpy(D2D_imagebuffer, CUDA_imagebuffer[0], KEPERNYO_WIDTH * KEPERNYO_HEIGHT * sizeof(unsigned int), cudaMemcpyDeviceToHost);
 terulet.left = 0;
 terulet.top = 0;
 terulet.right = KEPERNYO_WIDTH ;
 terulet.bottom = KEPERNYO_HEIGHT ;
 memkeptarolo->CopyFromMemory(&terulet, D2D_imagebuffer, KEPERNYO_WIDTH * sizeof(unsigned int));
 pRT->BeginDraw();
...
void PEGA_add_2D_point(int x, int y, int color)
{
 ...
 if (x >= KEPERNYO_WIDTH || y >= KEPERNYO_HEIGHT || x < 0 || y < 0) return;
...
void PEGA_add_2D_line(int x1, int y1, int x2, int y2, int color)
{
 ...
 if (x1 >= KEPERNYO_WIDTH) x1 = KEPERNYO_WIDTH-1;
 else if (x1 < 0) x1 = 0;
 if (y1 >= KEPERNYO_HEIGHT) y1 = KEPERNYO_HEIGHT-1;
 else if (y1 < 0) y1 = 0;
 if (x2 >= KEPERNYO_WIDTH) x2 = KEPERNYO_WIDTH-1;
 else if (x2 < 0) x2 = 0;
 if (y2 >= KEPERNYO_HEIGHT) y2 = KEPERNYO_HEIGHT-1;
 else if (y2 < 0) y2 = 0;
...
void PEGA_add_2D_triangle(int x1, int y1, int x2, int y2, int x3, int y3, int color)
{
 ...
 if (x3 >= KEPERNYO_WIDTH) x3 = KEPERNYO_WIDTH - 1;
 else if (x3 < 0) x3 = 0;
 if (y3 >= KEPERNYO_HEIGHT) y3 = KEPERNYO_HEIGHT - 1;
 else if (y3 < 0) y3 = 0;
...
void PEGA_clearscreen(void)
{
 ...
 {
  cudaSetDevice(i);
  cudaMemset(CUDA_imagebuffer[i], 255, KEPERNYO_HEIGHT * KEPERNYO_WIDTH * sizeof(unsigned int));
  cudaMemset(CUDA_maskimagebuffer[i], 0, KEPERNYO_HEIGHT * KEPERNYO_WIDTH * sizeof(unsigned int));
 }
}
